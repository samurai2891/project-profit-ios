...
> **作成日**: 2026-02-24
> **最終更新**: 2026-02-25 (Round 1 修正完了: 7件)
...
| 優先度 | 合計 | 修正済み | 残り |
|--------|------|----------|------|
| CRITICAL | 9 | 5 (C1,C2,C3,C6,C7) + 1部分(C5) | 3 |
| HIGH | 11 | 0 | 11 |
| MEDIUM | 15 | 1 (M14) | 14 |
| 構造的欠落 | 7 | 0 | 7 |
...
- **状態**: [ ] 未修正
- **ファイル**: `ProjectProfit/Services/DataStore.swift`
- **行番号**: L200-204 (deleteProject), L219-222 (recurring部分), L364-365 (deleteTransaction), L552-553 (deleteRecurring)
...
- **状態**: [x] 部分修正済み (2026-02-25, commit 266d01c) — lastError の UI 表示を追加。ロールバックは未対応。
- **修正内容**: MainTabView に DataStore.lastError のエラーアラートを追加。Binding<Bool> パターンで全タブから捕捉可能。テスト2件追加。
- **ファイル**: `ProjectProfit/Services/DataStore.swift`
...
- **状態**: [ ] 未修正
- **ファイル**: `ProjectProfit/Services/DataStore.swift`
- **行番号**: L117-127 (addProject), L193-237 (deleteProject)
...
- **状態**: [ ] 未修正
- **ファイル**: `ProjectProfit/Services/DataStore.swift`
- **行番号**: L929-939 (lastEligibleMonth計算), L955-960 (skipハンドリング), L964 (端数加算)
...
- **行番号**: L52-63 (incomeTotal/expenseTotal), L132-136 (totalByType)
...
- **状態**: [ ] 未修正
- **ファイル**: `ProjectProfit/Services/DataStore.swift` L1318-1335
...
### Phase 1: データ整合性の基盤修正（最優先）

| 順序 | ID | 概要 | 依存関係 |
|------|-----|------|----------|
| 1 | C1 | catch-up ループ実装 | なし |
| 2 | C2 | allocation 端数処理統一（9箇所） | なし |
| 3 | C5 | save() エラーの UI 表示 | なし |
| 4 | C4 | レシート画像削除順序修正 | C5 |
| 5 | C6 | deleteRecurring の生成済みトランザクション処理 | なし |
| 6 | C9 | 削除済みトランザクションの再生成対応 | C1 |
| 7 | C3 | カテゴリ削除の参照チェック | なし |
| 8 | C8 | 頻度変更時の lastGeneratedDate リセット | C1 |
| 9 | H5 | reverseCompletionAllocations 端数処理 | C2 |
| 10 | H7 | refresh*() のエラーハンドリング | C5 |

### Phase 2: 会計正確性の修正

| 順序 | ID | 概要 | 依存関係 |
|------|-----|------|----------|
| 11 | C7 | fiscal year デフォルト修正 | なし |
| 12 | H4 | monthlySpread 年途中除数修正 | なし |
| 13 | H3 | monthlySpread skipDates + 端数 | なし |
| 14 | H8 | updateTransaction 後の pro-rata 再計算 | なし |
| 15 | H11 | フィルタ時合計値修正 | なし |
| 16 | H6 | pro-rata 全0日エッジケース | なし |
| 17 | H1 | equalAll 対称性修正 | なし |
| 18 | H9 | プロジェクト削除の履歴保護 | T5 |
| 19 | H10 | equalAll 再処理のユーザー編集保護 | なし |
| 20 | H2 | 通知機能の接続 | なし |
...
| 順序 | ID | 概要 |
|------|-----|------|
| 21 | M14 | パストラバーサル脆弱性修正 |
| 22 | M2 | SwiftData マイグレーション戦略 |
| 23 | M1 | Optional 型の非Optional化 (M2後) |
| 24 | M5 | ratio 範囲バリデーション |
| 25 | M6 | monthOfYear 範囲バリデーション |
| 26 | M4 | categoryId 空文字禁止 |
| 27 | M10 | カテゴリ名一意性チェック |
| 28 | M11 | 重複プロジェクト選択防止 |
| 29 | M15 | 日付整合性バリデーション |
| 30 | M7 | レシート画像更新の安全化 |
| 31 | M13 | deleteAllData のレシート画像完全削除 |
| 32 | M12 | endDate の過去日設定許可 |
| 33 | M8 | CSV エクスポート全データオプション |
| 34 | M9 | CSV フィールド拡充 |
