...
struct PostingIntakeUseCase {
    ...
    private let fixedAssetWorkflowUseCase: FixedAssetWorkflowUseCase

    init(modelContext: ModelContext) {
        let queryUseCase = TransactionFormQueryUseCase(modelContext: modelContext)
        self.modelContext = modelContext
        self.store = PostingIntakeStore(modelContext: modelContext)
        self.transactionFormQueryUseCase = queryUseCase
        self.postingSupport = CanonicalPostingSupport(
            modelContext: modelContext,
            transactionFormQueryUseCase: queryUseCase
        )
        self.fixedAssetWorkflowUseCase = FixedAssetWorkflowUseCase(modelContext: modelContext)
    }
    ...
    func saveManualCandidate(input: ManualPostingCandidateInput) async throws -> PostingCandidate {
        ...
        let candidate = postingSupport.candidateWithProjectAllocations(
            posting.candidate.updated(status: .draft),
            allocations: input.type == .transfer ? [] : input.allocations
        )

        do {
            try await store.saveCandidate(candidate)
            return candidate
        } catch {
    ...
    func importTransactions(request: CSVImportRequest) async -> CSVImportResult {
        ...
        case .settingsTransactionCSV:
            return await importTransactionCSV(request: request)
    ...
    private func importTransactionCSV(request: CSVImportRequest) async -> CSVImportResult {
        ...
        for entry in parsedEntries {
            let allocations: [(projectId: UUID, ratio: Int)]
            do {
                allocations = try resolvedImportAllocations(for: entry, projects: &projects)
            } catch {
            ...
            do {
                ...
                try await store.saveCandidate(queuedCandidate)
                candidateCount += 1
            } catch {
                lineErrors.append(CSVImportLineError(line: entry.sourceLine, reason: error.localizedDescription))
            }
    ...
    private func createEvidence(
        ...
        if try store.existingEvidenceId(businessId: businessId, fileHash: fileHash) != nil {
    ...
    private func resolvedImportAllocations(
        ...
        return try entry.allocations.map { allocation in
            ...
            let created = try store.createProject(name: allocation.projectName)
